...
Result<std::string> probe_peer_cert_sha256(std::string_view host, uint16_t port,
  ...
  return fail("本次构建未启用 OpenSSL，无法探测证书指纹（--probe-cert 不可用）");
}
...
std::size_t probe_node_certificates(NodeList& nodes, int timeout_seconds,
  ...
  (void)timeout_seconds;
  if (warnings != nullptr) {
    warnings->push_back("本次构建未启用 OpenSSL，--probe-cert 被忽略");
  }
  return 0;
}

#endif  // SUBCONV_HAVE_OPENSSL
